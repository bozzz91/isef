...
public interface TaskRepo extends JpaRepository<Task, Long> {
    ...
    public List<Task> findByOwnerOrderByModificationTimeAsc(Person p);
    
    public List<Task> findByStatusOrderByModificationTimeAsc(Status p);
    ...
    @Query("select t from Task t LEFT JOIN t.executors c WHERE t.status.id = 3 and t.owner <> ?1 and (c.pk.person <> ?1 or c.pk.person is null)")
    ...
    @Query("from Task t WHERE t.status = ?2 and t.owner = ?1")
    public List<Task> findMyTasksByStatus(Person p, Status st);
}
